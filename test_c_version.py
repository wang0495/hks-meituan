...
- 通用标准（非美食）：
  ...
  - 7-8: 涵盖3种大类
  - 5-6: 只有1-2种大类，或同类型重复（如3个公园+1个餐厅）
  - 3-4: 几乎单一类型（如5个景点全是公园）
  - 0-2: 完全没有多样性
- 美食专用标准：
  - 9-10: 3种以上不同餐饮子类型（如海鲜正餐+港式茶餐厅+甜品+街边小吃）
  - 7-8: 2-3种餐饮子类型
  - 5-6: 有一定变化但子类型单一（如海鲜排档+海鲜市场+海鲜夜市都算"海鲜"一类）
  - 3-4: 完全同一类型（如全是海鲜排档或全是夜市）
  - 0-2: 毫无变化
