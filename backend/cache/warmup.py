...
使用方式::
    ...
    # 定时预热（后台任务）
    asyncio.create_task(warmup.start_background_warmup(interval=3600))

    # 按需预热
    await warmup.warmup_task("poi")
"""
...
# 预热任务类型：接收 cache 参数的异步可调用
WarmupTask = Callable[[MultiLevelCache], Coroutine[Any, Any, None]]
...
class CacheWarmup:
    ...
    3. 按需预热 -- 调用 ``warmup_task(name)`` 执行单个任务

    Args:
        cache: MultiLevelCache 实例，预热任务通过此参数访问缓存
    """
    ...
    def register(self, name: str, task: WarmupTask) -> None:
        ...
        Args:
            ...
            task: 接收 MultiLevelCache 参数的异步可调用对象
    ...
    async def warmup_task(self, name: str) -> WarmupResult:
        """执行单个预热任务（按需预热）。
        ...
        if name not in self._tasks:
            raise KeyError(f"预热任务 '{name}' 未注册，可用任务: {self.task_names}")
...
async def warmup_poi_cache(cache: MultiLevelCache) -> None:
    """预热 POI 数据到多级缓存。
...
async def warmup_city_category_cache(cache: MultiLevelCache) -> None:
    """预热城市-类别交叉索引到多级缓存。
...
_warmup: CacheWarmup | None = None


def get_cache_warmup() -> CacheWarmup:
    ...
    if _warmup is None:
        ...
        cache = get_multilevel_cache()
        _warmup = CacheWarmup(cache)
        _register_default_tasks(_warmup)
    return _warmup


def reset_cache_warmup() -> None:
    """重置全局预热器（用于测试）。"""
    global _warmup
...
def _register_default_tasks(warmup: CacheWarmup) -> None:
    """注册默认预热任务（按依赖顺序）。"""
