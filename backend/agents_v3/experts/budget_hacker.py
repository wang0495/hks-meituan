...
def _filter_budget_pois(candidates: list[dict]) -> list[dict]:
    """筛选免费/低价POI。"""
    budget_pois = []
    for c in candidates:
        cat = c.get("category", "")
        if cat in _EXCLUDE_CATS or _is_likely_macau(c.get("name", "")) or c.get("rating") is None:
            continue
        price = c.get("avg_price", 0)
        if price == 0 or price <= _PRICE_CAP or cat in _BUDGET_CATEGORIES:
...
def _build_budget_system_prompt(group_type: str) -> str:
    ...
    return f"""你是穷游规划专家。用户预算极低，请选择免费或超值景点和餐饮。
...
def _match_budget_picks(picks: list[dict], budget_pois: list[dict]) -> list[dict]:
    """匹配LLM picks到预算POI。"""
    proposals = []
...
async def budget_hacker(state: TravelState) -> dict:
    ...
    budget_pois = _geo_filter_budget_pois(_filter_budget_pois(candidates), state)
    if not budget_pois:
        return {"proposals": []}

    summaries = _build_budget_summaries(budget_pois)
