...
# ---------------------------------------------------------------------------
# Food subcategory definitions - imported from base.py
# _FOOD_SUBCATS, _FOOD_CATEGORIES, _FOOD_KEYWORDS, _LIANGCHA_KEYWORDS
...
def _filter_food_expert_pois(all_pois: list[dict]) -> list[dict]:
    ...
    return [
        ...
        for c in all_pois
        if (
            any(kw in c.get("category", "") for kw in _FOOD_CATEGORIES)
            or any(kw in c.get("name", "") for kw in _FOOD_KEYWORDS)
        )
...
async def _prepare_food_expert_data(
    ...
    if len(foods) < 3:
        ...
        for c in state.get("candidates", []):
            if (
                any(kw in c.get("name", "") for kw in _FOOD_KEYWORDS)
