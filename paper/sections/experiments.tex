...
\begin{enumerate}[leftmargin=*]
    \item \textbf{LLM Judge}: An independent LLM evaluator (DeepSeek Chat via API) with scenario-aware scoring rubrics scores each route on four dimensions (0--10 scale): intent match, POI quality, geographic continuity, and scene diversity. The overall score is the unweighted average. We run each evaluation 2 times and report the lower-score run (conservative estimate) to reduce LLM scoring variance~\cite{gu2024llmjudge}.
...
\begin{table}[H]
\centering
\caption{Model comparison. Pass rate and overall score from multi-model A/B test (commit \texttt{edfe2c0}); xopqwen35v35b row from 2026-06-02 test. Pricing as of 2025-05.}
\label{tab:model_compare}
\begin{tabular}{@{}lccp{2.5cm}@{}}
\toprule
\textbf{Model} & \textbf{Pass} & \textbf{Overall} & \textbf{Price} \\
\midrule
DeepSeek Chat & 4/5 & 6.4 & \$\textasciitilde\$1/M tokens \\
...
\midrule
xopqwen35v35b (iFlytek) & \textbf{5/5} & \textbf{7.4} & \textbf{\$\textasciitilde\$0.15/M tokens} \\
\bottomrule
\end{tabular}
\end{table}
...
Hallucination Rate & 0.0\% & 22.1\% & 0.0\% & 0.0\% \\
