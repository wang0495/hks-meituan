...
\resizebox{\columnwidth}{!}{\begin{tikzpicture}[
    ...
    wave/.style={rectangle, rounded corners, draw=black!60, fill=black!8, minimum width=1.8cm, minimum height=0.7cm, font=\small, align=center},
...
Wave~1 experts execute in parallel with no inter-expert communication; each receives the same shared state (user intent, candidate POIs, scene type) and produces independent proposals. Table~\ref{tab:experts} lists the expert modules by execution wave. Wave~2 experts read the \textit{merged} Wave~1 proposals from shared state---for example, the Traffic Expert computes distance matrices over Wave~1's candidate POIs, and the Local Expert avoids recommending places already selected by the POI Expert. This is \textit{data-flow dependency}, not direct negotiation: experts do not exchange messages or iterate toward consensus.
...
Given the winning candidate, the Synthesizer applies post-processing: greedy nearest-neighbor reordering for geographic coherence, breathing-space insertion for slack, and a climax-stage finishing heuristic~\cite{kahneman1993when} that prioritizes high-excitement POIs near the route's end.
