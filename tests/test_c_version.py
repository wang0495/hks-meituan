...
**intent_match** (意图匹配):
- 美食型：路线以餐饮为主就是好匹配，不需要大量景点。选了好吃且多样的餐厅→8-9
- 目的地型：只要包含了用户指定的核心目的地，即算高分（7-9）。POI数量少不代表匹配差
...
- 美食型：质量主要看餐厅口碑和菜品评价，不要求景点质量
- 9-10: 都是值得专程去的  |  7-8: 大部分不错  |  5-6: 一般  |  3-4: 偏低  |  0-2: 不值得

**geo_continuity** (地理合理性):
- 美食型：餐厅可以分散（美食探索本身就是目的），但不应来回折返。同区域连走即可
- 目的地型：POI集中在同一区域是优点（如都在长隆片区），给8-9
- 特种兵型：跨区域赶场是预期行为，不要因为跨度大就扣分。看单段是否折返即可
- 休闲型：景点少且距离近加分
- 观光型：看路线是否有不必要的折返
  - 9-10: 流畅  |  7-8: 基本合理  |  5-6: 有绕路  |  3-4: 不合理  |  0-2: 混乱

**scene_diversity** (场景多样性) — 按场景分化最严重的维度:
- 美食型：只看餐饮内部多样性！不需要景点/购物/文化！
  - 夜市/美食街/海鲜街属于综合性场所，内部已有多种小吃，算作"综合美食"一个类型
  - 海鲜正餐+茶餐厅+甜品+夜市 = 4种子类 = 满分(9-10)
  - 夜市+海鲜街+海鲜夜市 = 全是街/市场 = 低分(3-4)
  - 全餐饮路线本身OK，不需要加景点
  - 9-10: 4种以上不同餐饮子类型  |  7-8: 3种  |  5-6: 2种  |  3-4: 全同类型  |  0-2: 毫无变化
- 目的地型：多样性不是重点，主题一致性更重要（给7-8）
- 特种兵型：覆盖多种类型景点加分（自然+文化+娱乐+地标，每多一类+1）
- 休闲型：不要求多样性，1-2种大类就够（给7-8）
- 观光型：通用标准
  - 9-10: 涵盖4种以上大类（景点+餐饮+文化+自然等），且每类都是精选
  - 7-8: 涵盖3种大类  |  5-6: 只有1-2种大类  |  3-4: 几乎单一类型  |  0-2: 完全没有
